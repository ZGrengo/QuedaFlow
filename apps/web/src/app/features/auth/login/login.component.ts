import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';
import { MatCardModule } from '@angular/material/card';
import { MatButtonModule } from '@angular/material/button';
import { ActivatedRoute, Router, RouterModule } from '@angular/router';
import { AuthService } from '../../../core/services/auth.service';

@Component({
  selector: 'app-login',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
    MatCardModule,
    MatButtonModule
  ],
  template: `
    <div class="qf-page login-container">
      <mat-card class="qf-surface login-card">
        <mat-card-header>
          <mat-card-title>QuedaFlow</mat-card-title>
          <mat-card-subtitle>Encuentra huecos de horarios entre compañeros</mat-card-subtitle>
        </mat-card-header>
        <mat-card-content>
          <div *ngIf="message" class="message" [class.error]="isError">
            {{ message }}
          </div>

          <button mat-raised-button class="google-btn full-width" type="button" [disabled]="loading" (click)="onSignInWithGoogle()">
            <span class="btn-content">
              <svg class="google-icon" viewBox="0 0 24 24" aria-hidden="true">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              {{ loading ? 'Conectando...' : 'Continuar con Google' }}
            </span>
          </button>
        </mat-card-content>
      </mat-card>
    </div>
  `,
  styles: [`
    .login-container {
      display: flex;
      justify-content: center;
      align-items: center;
      min-height: 100vh;
      box-sizing: border-box;
    }

    .login-card {
      width: 100%;
      max-width: 400px;
    }

    .full-width {
      width: 100%;
    }

    .google-btn {
      background: #fff;
      color: #3c4043;
      border: 1px solid #dadce0;
    }

    .google-btn:hover:not([disabled]) {
      background: #f8f9fa;
      border-color: #dadce0;
    }

    .btn-content {
      display: inline-flex;
      align-items: center;
      gap: 12px;
    }

    .google-icon {
      width: 20px;
      height: 20px;
    }

    .message {
      margin: 0 0 16px 0;
      padding: 12px;
      border-radius: 4px;
      background-color: rgba(162, 211, 194, 0.25);
      color: #1a5c4a;
    }

    .message.error {
      background-color: rgba(203, 37, 70, 0.12);
      color: var(--qf-primary);
    }

    mat-card-content {
      padding-top: 16px;
    }
  `]
})
export class LoginComponent {
  loading = false;
  message = '';
  isError = false;

  constructor(
    private authService: AuthService,
    private router: Router,
    private route: ActivatedRoute
  ) {
    // Si ya hay sesión (p. ej. llegó desde OAuth), ir a returnUrl o dashboard
    this.authService.getCurrentUser().subscribe(user => {
      if (user) {
        const returnUrl = this.route.snapshot.queryParamMap.get('returnUrl');
        const target = returnUrl && returnUrl.startsWith('/') ? returnUrl : '/dashboard';
        this.router.navigateByUrl(target, { replaceUrl: true });
      }
    });
  }

  async onSignInWithGoogle() {
    this.loading = true;
    this.message = '';
    this.isError = false;

    try {
      const returnUrl = this.route.snapshot.queryParamMap.get('returnUrl') ?? undefined;
      await this.authService.signInWithGoogle(returnUrl);
      // La redirección la hace Supabase/Google; si no hay error, el usuario sale de la página
    } catch (error: any) {
      this.message = error.message || 'Error al iniciar sesión con Google';
      this.isError = true;
    } finally {
      this.loading = false;
    }
  }
}
